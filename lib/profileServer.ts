...
export async function getProfileForUser(user: User): Promise<AltrProfile> {
  ...
  return {
    ...
    plan: entitlement.planId, trainingProgress, tone: profile?.tone ?? preferences?.default_tone ?? "balanced",
    onboardingCompleted: profile?.onboarding_completed ?? false, stats,
