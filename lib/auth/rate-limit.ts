...
const limits: Record<AuthRateLimitAction, { attempts: number; windowSeconds: number }> = {
  ...
  // Prompt 040 — a cheap paginated read (GET /api/ai/drafts, history list),
  // not a generation call: budgeted like `import_chunk`'s read-ish
  // generosity, deliberately far above `ai_generation`'s 30/hour so
  // browsing draft history never competes with that separate, expensive
  // budget.
  ai_drafts_list: { attempts: 120, windowSeconds: 60 * 60 },
