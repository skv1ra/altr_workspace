...
export type EditableProfileUpdate = Partial<Pick<AltrProfile, "name" | "role" | "altrName" | "bio" | "tone" | "preferences" | "onboardingCompleted">>;
export async function updateCurrentProfile(update: Partial<AltrProfile>) { const safe: EditableProfileUpdate = { name: update.name, role: update.role, altrName: update.altrName, bio: update.bio, tone: update.tone, preferences: update.preferences, onboardingCompleted: update.onboardingCompleted }; return api<{ profile: AltrProfile }>("/api/me", { method: "PATCH", body: JSON.stringify(safe) }).then((payload) => payload.profile); }
